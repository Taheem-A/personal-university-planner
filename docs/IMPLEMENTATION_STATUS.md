...
**Milestone 3 — IN PROGRESS.** The planner contract, architecture, normalization/candidate-capacity, prioritization and sustainable-allocation slices are implemented on `plan/milestone-3-planner-core-v1`. The core has focused pure modules for normalization, eligibility, occupied timeline, candidate windows, pressure/ranking, allocation, sustainability, validation, simulation and versioning. Its explicit snapshot includes the facts needed by later Milestone-3 stages, and output carries `heuristic-v1`. See [ADR 0005](./decisions/0005-planner-core-v1-contract.md). This is not the complete scheduling engine or the Milestone-3 exit gate.
...
**Milestone-3 planner policy slice:** dependencies, context, commute, weekend bias, manual intent, locks and stability. Extend the existing eligibility/capability foundations into a coherent placement and replan policy, then complete repair, explanations and the scenario/property suite before the gate.
