...
**Milestone 3 — GATE PASSED locally.** The [literal exit-gate audit](./milestone-3-exit-gate.md) maps every roadmap requirement to implementation and passing test evidence. The planner contract, architecture, normalization/candidate-capacity, prioritization, sustainable-allocation, policy, validation/repair/scenario, canonical regression and randomized-invariant slices are implemented on `plan/milestone-3-planner-core-v1`. The core has focused pure modules for normalization, eligibility, occupied timeline, candidate windows, pressure/ranking, allocation, sustainability, retention policy, validation, repair, diagnostics, simulation and versioning. Its explicit snapshot carries all facts needed by `heuristic-v1`. See [ADR 0005](./decisions/0005-planner-core-v1-contract.md). GitHub CI is separate from the local gate evidence; the draft PR remains unmerged.
...
**Milestone 4 — Planner Service, PlannerRuns, Persistence, and Incremental Replanning.** Milestone 4 has **not started**. The Milestone-3 draft PR must remain unmerged pending review.
