...
The [canonical scenario matrix](./milestone-3-scenario-matrix.md) now covers all 13 roadmap scenarios with permanent synthetic Toronto fixtures. The targeted suite passed 13/13 locally. It exposed and fixed a soft-policy allocation defect: a preferred free-time reserve could strand otherwise feasible work before an earlier deadline. The planner now retries that task without the soft daily preference when needed, preserving hard constraints and warning about the compromise. Randomized property/invariant testing, determinism/version proof and the literal Milestone-3 exit audit remain open.
...
**Milestone-3 final proof slice:** randomized property/invariant testing, deterministic repeated-run and planner-version proof, then a literal exit-gate audit. Do not mark the milestone complete until every gate criterion passes.
