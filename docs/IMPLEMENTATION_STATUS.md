...
**Milestone 4 — IN PROGRESS.** The first slice added the typed Planner Service and canonical-state to `PlannerInput` assembly under a repeatable-read snapshot. The second slice adds a database-backed per-user planning revision, guarded claims, durable trigger idempotency, guarded PlannerRun lifecycle, and transaction-oriented generated-session batch/supersession primitives. PostgreSQL triggers invalidate snapshots for planning-relevant canonical changes while leaving unrelated profile/text edits alone. Migration `0008_planner_authority_substrate` is tracked and replayed from zero and from the Milestone-3 schema. [ADR 0006](./decisions/0006-planner-service-input-assembly.md) records both slices. The service still generates only in memory; it does not yet persist an authoritative plan. **Exact next slice: authoritative planner execution and transactional plan persistence.**

Full local `pnpm verify`, `pnpm test:planner-scenarios` (13/13), `pnpm test:planner-properties` (2/2), and `pnpm check:dependencies` passed for this slice. The new migration replay and repository tests pass in ordinary CI.
...
**authoritative planner execution and transactional plan persistence.** Milestone 4 remains **IN PROGRESS**.
