...
**Milestone 3 — IN PROGRESS.** The planner contract, architecture, normalization/candidate-capacity, prioritization, sustainable-allocation and policy slices are implemented on `plan/milestone-3-planner-core-v1`. The core has focused pure modules for normalization, eligibility, occupied timeline, candidate windows, pressure/ranking, allocation, sustainability, retention policy, validation, simulation and versioning. Its explicit snapshot includes the facts needed by later Milestone-3 stages, and output carries `heuristic-v1`. See [ADR 0005](./decisions/0005-planner-core-v1-contract.md). This is not the complete scheduling engine or the Milestone-3 exit gate.
...
**Milestone-3 validation and repair slice:** complete hard/soft validation, workload-conserving repair, explicit infeasibility and reason codes, and non-mutating scenarios. Then run the broader property/invariant suite before the gate.
