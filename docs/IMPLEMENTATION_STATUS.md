...
**Milestone 4 — IN PROGRESS.** Canonical input assembly, revision/idempotency protection, authoritative `heuristic-v1` execution, trigger orchestration and transactional PlannerRun/session history are implemented. Production Today and Week application read models now project active canonical sessions, fixed/recurring commitments, protected and commute windows, remaining work, risk, and plan state from one scoped snapshot. Durable session reason codes and safe plan-history projections support future explanations. Authenticated read routes are adapters only; the approved UI remains unchanged. [ADR 0006](./decisions/0006-planner-service-input-assembly.md) records the policies. **Exact next step: Milestone-4 final acceptance, live PostgreSQL proof, CI and exit-gate audit.**

Full local `pnpm verify` passed for this read-contract slice: formatting, lint, package boundaries, typecheck, unit and integration tests, schema validation, production build with all three planner read routes, and 2 Chromium E2E tests. `pnpm test:planner-scenarios` (13/13), `pnpm test:planner-properties` (2/2), and `pnpm check:dependencies` also passed. The guarded live PostgreSQL race command and end-to-end planner run remain for the Milestone-4 acceptance gate.
