...
**Milestone 4 — IN PROGRESS.** The first slice adds a typed Planner Service contract and authenticated canonical-state to `PlannerInput` assembly. Planning reads use one repeatable-read database snapshot, with a user-scoped, bounded planning repository. The application layer selects a seven-local-day exact horizon from explicit `now`, maps persisted tasks, dependencies, events, recurring course meetings, availability, protection, sleep, preferences and active sessions, and returns structured input failures when required facts are absent. The service can call `heuristic-v1` in memory; it does not yet persist PlannerRuns or generated sessions. [ADR 0006](./decisions/0006-planner-service-input-assembly.md) records the mapping and minimum-sleep source. **Exact next slice: Planner persistence/concurrency substrate.**

Full local `pnpm verify`, `pnpm test:planner-scenarios` (13/13), `pnpm test:planner-properties` (2/2), and `pnpm check:dependencies` passed for this slice.
...
**Planner persistence/concurrency substrate.** Milestone 4 remains **IN PROGRESS**.
