...
**Milestone 4 — IN PROGRESS.** Canonical input assembly, the revision/idempotency substrate, authoritative `heuristic-v1` execution, transactional PlannerRun/session history and post-commit trigger orchestration are implemented. The concurrency slice adds overlapping-run, canonical-race, duplicate-event and abandoned-run recovery coverage. A late recovered computation cannot commit, and a guarded disposable PostgreSQL race command is available for the acceptance gate. [ADR 0006](./decisions/0006-planner-service-input-assembly.md) records the policies. **Exact next slice: production Today/Week planner view models and plan-history read services.**

Full local `pnpm verify` passed for this slice: 131 unit tests, 39 integration tests, strict checks, production build and 2 Chromium E2E tests. `pnpm test:planner-scenarios` (13/13), `pnpm test:planner-properties` (2/2), and `pnpm check:dependencies` also passed. The migration replay and repository tests pass in ordinary CI; the guarded live PostgreSQL race command and end-to-end planner run remain for the Milestone-4 acceptance gate.
