...
**Milestone 5 — foundation slice in progress on `web/milestone-5-production-ui`.** The Milestone-0 home placeholder has been replaced by a redirect to the protected Today route. A production AppShell, signed-out Google sign-in entry, route scaffold, light/dark tokens, IBM Plex type, Lucide icons, restrained Motion panel behavior, responsive navigation, URL-driven panel host and shared domain visual primitives are implemented. These routes deliberately show truthful empty shells until their real-service screen slices are built. No production UI imports approved-preview fixtures. This branch is not merged, and Milestone 5 is not complete. **Exact next slice: production Today migration against the authenticated Today read model.** Week, Upcoming, Inbox, course, availability, integration, settings, Planner, scenario/conflict, onboarding and full responsive screen work remain in Milestone 5; Calendar integration and natural-language Planner execution remain deferred to later milestones.
