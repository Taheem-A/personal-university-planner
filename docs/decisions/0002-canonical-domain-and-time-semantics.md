...
| ExternalObjectMap  | Provider-to-canonical identity            | Global provider/external uniqueness collided across users      | Account- and user/provider-scoped uniqueness with sync provenance                    | Composite account/user/provider foreign key prevents provider mismatch |
...
The generated initial SQL is extended with named PostgreSQL `CHECK` constraints for the scalar, range, date-order, interval, and self-reference invariants Prisma cannot express. `ExternalObjectMap` now references the integration account by `(id, userId, provider)`, closing the provider-consistency gap before migration history is frozen. These constraints are migration-owned and are a further reason `prisma db push` cannot replace source-controlled history.
