...
exports.deriveEstimateLearning = deriveEstimateLearning;
 ...
 */
function deriveEstimateLearning(observations) {
    const usable = observations.filter((item) => !item.atypical && item.estimatedMinutes > 0 && item.actualMinutes > 0);
    if (usable.length === 0) {
